...
def main() -> int:
    with tempfile.TemporaryDirectory() as temporary:
        ...
        try:
            ...
            app.update_manifest_url_var.set("file:///mock-manifest.json")
            ...
            assert str(app.update_download_button.cget("state")) == "normal"
            ...
            with patch("check_vehicle_ocr.app.download_verified", return_value=destination):
                app.download_update()
                _pump(app, lambda: str(destination) in app.update_status_var.get())
            assert "SHA-256" in app.update_status_var.get() and str(destination) in app.update_status_var.get()
        finally:
            app.destroy()
